...
func (m *tuiModel) handleInput(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		...
		case value == "/help":
			...
			s.appendLine(line{Type: lineInfo, Data: "Tab: focus tab bar  Shift+←/→: switch tabs  Alt+1..9: jump to tab"})
...
func (m tuiModel) buildTabInfos() []tabInfo {
	...
	for i, s := range m.sessions {
		...
		if s.state == tuiStatePrompt {
			...
			// Pulsing blue dot: bounces between three shades.
			pulseColors := []string{"75", "33", "27", "33"}
